(() => {
    ...
    function assessImportEntry(entry) {
      ...
      const canLookup = Boolean(company.cnpj && validCnpj && window.cnpjService);
      const lookupFinished = Boolean(entry.enrichmentAttempted);
      ...
      if (company.situacao && !situacoesContato.includes(company.situacao)) {
        warnings.push('Situação não padronizada');
      }

      const requiredAfterLookup = [
      ...
      requiredAfterLookup.forEach(([field, message]) => {
        if (String(company[field] || '').trim()) return;
        if (canLookup && !lookupFinished) {
          warnings.push(`${message} — será consultada pelo CNPJ`);
        } else {
          issues.push(message);
        }
      });

      if (!company.cnaePrincipal && canLookup && !lookupFinished) {
        warnings.push('CNAE será consultado pelo CNPJ');
      } else if (!company.cnaePrincipal && lookupFinished) {
        warnings.push('CNAE não localizado nas fontes públicas');
      }

      if (!company.polo) {
        if (!lookupFinished) {
          warnings.push('Polo será preenchido como "Não informado"');
        } else {
          issues.push('Polo não informado');
        }
      } else if (entry.defaultedPolo) {
        warnings.push('Polo preenchido automaticamente como "Não informado"');
      }
    ...
    function renderImportPreview() {
      ...
      if (confirm) confirm.disabled = !state.importRows.some((entry) => !entry.issues.length) || state.importRunning || state.importPreparing;
    }
    ...
    async function handleSpreadsheetFile(file) {
      ...
      if ($('#importSummary')) {
        $('#importSummary').innerHTML = '<div class="summary-box"><i class="fa-solid fa-spinner fa-spin"></i> Lendo a planilha…</div>';
      }
      ...
      try {
        ...
        if (!state.importRows.length) {
          throw new Error('Nenhuma linha de dados foi identificada.');
        }
    ...
    async function enrichImportEntry(entry, { retry = false } = {}) {
      ...
      if (
        ...
        && missingPublicData
        && window.cnpjService
        && (!entry.enrichmentAttempted || retry)
      ) {
        try {
    ...
    async function prepareImportRows() {
      ...
      updateImportProgress(0, total, `Consultando os CNPJs de ${total} linha(s)…`);
      ...
      for (let index = 0; index < total; index += 1) {
        ...
        updateImportProgress(
          index,
          total,
          `Consultando CNPJ da linha ${entry.rowNumber} — ${index + 1} de ${total}…`
        );
        ...
        updateImportProgress(
          index + 1,
          total,
          `${index + 1} de ${total} CNPJ(s) processado(s).`
        );

        if (index < total - 1) await waitImportLookup(550);
      }
    }
    ...
    async function confirmImport() {
      ...
      try {
        for (let index = 0; index < state.importRows.length; index += 1) {
          const entry = state.importRows[index];
          let company = normalizeCompany(entry.company);
          updateImportProgress(index, state.importRows.length, `Importando linha ${entry.rowNumber} — ${index + 1} de ${state.importRows.length}…`);

          try {
            const retryLookup = Boolean(entry.lookupError);
            if (!entry.enrichmentAttempted || retryLookup) {
              await enrichImportEntry(entry, { retry: retryLookup });
              company = normalizeCompany(entry.company);
              if (retryLookup && index < state.importRows.length - 1) {
                await waitImportLookup(550);
              }
            } else {
            ...
            if (entry.issues.length) {
              throw new Error(entry.issues.join('; '));
            }
    ...
    function bindEvents() {
      ...
      $('#importBtn').onclick=()=>{
        ...
        state.importRows=[];state.importErrors=[];state.importFileName='';state.importPreparing=false;
