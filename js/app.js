(() => {
    ...
    function normalizeCompany(company) {
      ...
      obj.vigenciaResumo = vigenciaInfo(obj.inicioVigencia, obj.fimVigencia).resumo;
      return obj;
    }
    ...
    function getFilteredCompanies() {
      ...
      return state.data.concedentes.map(normalizeCompany).filter(c=>{
        ...
        const cmp = typeof av==='number' ? av-bv : String(av).localeCompare(String(bv),'pt-BR',{numeric:true});
    ...
    function renderCompanies() {
      ...
      </tr>`).join('') : `<tr><td colspan="17">${emptyState('Nenhum cadastro encontrado','Ajuste os filtros ou cadastre uma nova concedente.')}</td></tr>`;
    ...
    function openContactForm(companyId=null) {
      $('#contactForm').reset(); $$('#contactForm .invalid').forEach(e=>e.classList.remove('invalid')); refreshContactSelect();
      $('#contactDate').value=todayISO(); $('#contactTime').value=new Date().toTimeString().slice(0,5); $('#contactCompanyId').value=companyId||''; $('#contactCompanySelect').value=companyId||'';
      openModal('contactModalBackdrop');
    }
    async function saveContact() {
      if(!canEdit()){toast('error','Acesso restrito','Seu perfil não pode registrar contatos.');return;}
      const companyId=$('#contactCompanySelect').value; const company=state.data.concedentes.find(c=>c.id===companyId);
      let valid=true; ['contactCompanySelect','contactDate','contactTime','contactResponsible','contactMethod','contactResult'].forEach(id=>{const el=$('#'+id);el.classList.toggle('invalid',!el.value.trim());if(!el.value.trim())valid=false;});
      if(!valid||!company){toast('error','Revise o formulário','Preencha os campos obrigatórios do contato.');return;}
      const contact={id:uid(),data:$('#contactDate').value,horario:$('#contactTime').value,responsavel:$('#contactResponsible').value.trim(),forma:$('#contactMethod').value,pessoa:$('#contactPerson').value.trim(),resultado:$('#contactResult').value,proximaAcao:$('#contactNextAction').value.trim(),proximaData:$('#contactNextDate').value,observacoes:$('#contactNotes').value.trim()};
      const submit=$('#contactForm button[type="submit"]');if(submit){submit.disabled=true;submit.innerHTML='<i class="fa-solid fa-spinner fa-spin"></i>Registrando...';}
      try{
        const saved=await window.remoteData.createContact(companyId,contact,company);
        company.contatos=company.contatos||[];company.contatos.push(saved);company.situacao=contact.resultado;if(!company.formasContato.includes(contact.forma))company.formasContato.push(contact.forma);company.updatedAt=new Date().toISOString();
        state.selectedContactCompanyId=companyId;closeModal('contactModalBackdrop');renderAll();applyAccessRules();switchPanel('contatos');toast('success','Contato registrado','O histórico e a situação foram salvos no banco online.');
      }catch(error){console.error(error);toast('error','Falha ao registrar contato',error.message||'Não foi possível salvar o contato.');}
      finally{if(submit){submit.disabled=false;submit.innerHTML='<i class="fa-solid fa-floppy-disk"></i>Registrar contato';}}
    }
    ...
    function renderSettings() { const contacts=state.data.concedentes.reduce((sum,c)=>sum+c.contatos.length,0); $('#storageSummary').textContent=`${state.data.concedentes.length} concedente(s) e ${contacts} contato(s) no Supabase.`; }
    ...
    function exportCompanies(mode){if(!ensureAdmin('exportar cadastros'))return;let rows=[];const now=parseDate(todayISO());if(mode==='all')rows=state.data.concedentes;if(mode==='filtered')rows=getFilteredCompanies();if(mode==='expired')rows=state.data.concedentes.filter(c=>calculateVigenciaStatus(c.fimVigencia)==='Vencido');if(mode==='upcoming')rows=state.data.concedentes.filter(c=>c.fimVigencia&&(()=>{const d=daysBetween(now,parseDate(c.fimVigencia));return d>=0&&d<=90;})());if(!rows.length){toast('warning','Nada para exportar','Nenhum registro corresponde à opção escolhida.');return;}const csv='\uFEFF'+[CSV_HEADERS,...rows.map(companyToCsvRow)].map(r=>r.map(csvEscape).join(';')).join('\r\n');downloadBlob(csv,'text/csv;charset=utf-8;',`renovacoes_convenios_${new Date().toLocaleDateString('pt-BR').replaceAll('/','-')}.csv`);closeModal('exportModalBackdrop');toast('success','Exportação concluída',`${rows.length} registro(s) exportado(s).`);}
    function exportContacts(){if(!ensureAdmin('exportar contatos'))return;const headers=['Concedente','CNPJ','Data do contato','Horário','Responsável','Forma de contato','Pessoa contatada','Resultado do contato','Próxima ação','Próximo contato','Observações'];const rows=state.data.concedentes.flatMap(c=>(c.contatos||[]).map(ct=>[c.nomeFantasia||c.razaoSocial,c.cnpj,formatDate(ct.data),ct.horario,ct.responsavel,ct.forma,ct.pessoa,ct.resultado,ct.proximaAcao,formatDate(ct.proximaData),ct.observacoes]));if(!rows.length){toast('warning','Sem contatos','Não há histórico para exportar.');return;}const csv='\uFEFF'+[headers,...rows].map(r=>r.map(csvEscape).join(';')).join('\r\n');downloadBlob(csv,'text/csv;charset=utf-8;',`historico_contatos_${new Date().toLocaleDateString('pt-BR').replaceAll('/','-')}.csv`);closeModal('exportModalBackdrop');toast('success','Histórico exportado',`${rows.length} contato(s) exportado(s).`);}
    function exportReportCSV(){if(!ensureAdmin('exportar relatórios'))return;const total=state.data.concedentes.length,renewed=state.data.concedentes.filter(c=>c.situacao==='Renovado').length,contacts=state.data.concedentes.reduce((s,c)=>s+c.contatos.length,0);const sections=[['Indicador','Valor'],['Total de convênios',total],['Renovações concluídas',renewed],['Percentual de renovação',total?`${((renewed/total)*100).toFixed(1).replace('.',',')}%`:'0,0%'],['Contatos realizados',contacts],['Concedentes sem contato',state.data.concedentes.filter(c=>!c.contatos.length).length],[],['Situação da Vigência','Total'],...Object.entries(countBy(state.data.concedentes,c=>c.situacaoVigencia)),[],['Situação do Contato','Total'],...Object.entries(countBy(state.data.concedentes,c=>c.situacao)),[],['Estado','Total'],...Object.entries(countBy(state.data.concedentes,c=>c.estado)),[],['Cidade','Total'],...Object.entries(countBy(state.data.concedentes,c=>c.cidade)),[],['Polo','Total'],...Object.entries(countBy(state.data.concedentes,c=>c.polo))];const csv='\uFEFF'+sections.map(r=>r.map(csvEscape).join(';')).join('\r\n');downloadBlob(csv,'text/csv;charset=utf-8;',`relatorios_convenios_${new Date().toLocaleDateString('pt-BR').replaceAll('/','-')}.csv`);toast('success','Relatório exportado','Os indicadores foram exportados em CSV.');}
    ...
    function downloadImportTemplate() {
      ...
      link.href = 'modelo/modelo_importacao_convenios.xlsx';
      link.download = 'modelo_importacao_convenios.xlsx';
      document.body.appendChild(link);
      link.click();
      link.remove();
      toast('success', 'Modelo preparado', 'A planilha formatada foi baixada. Preencha a aba Concedentes.');
    }
