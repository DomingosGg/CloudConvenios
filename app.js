(() => {
    ...
    async function exportBulkSelected() {
      ...
      const blob = workbookBlob([{ name:'Concedentes', rows:[CSV_HEADERS,...rows.map(companyToCsvRow)], widths:[23,19,34,28,16,22,28,24,12,28,10,20,20,10,22,30,18,22,18,18,16,18,24,24,28,24,24,34] }]);
    ...
    function renderManagementReport() {
      ...
      setOptions('#managementResponsible', companies.map((company)=>company.responsavelOperacional), $('#managementResponsible')?.value || '');
      setOptions('#managementState', companies.map((company)=>company.estado), $('#managementState')?.value || '');
      setOptions('#managementPolo', companies.map((company)=>company.polo), $('#managementPolo')?.value || '');
    ...
    function init() {
      loadData();setupStaticOptions();ensureWorkflowEnhancementsUI();ensureOutlookMessageUI();ensureAdvancedManagementUI();bindEvents();bindWorkflowEnhancementEvents();bindAdvancedManagementEvents();applyTheme();renderAll();updateMigrationSummary();applyAccessRules();
